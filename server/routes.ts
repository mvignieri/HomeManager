...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/analytics/tasks', async (req, res) => {
    try {
      ...
      res.json({
        kpi: { totalCreated, totalCompleted, completionRate, totalEffortHours: Math.round(totalEffortHours * 10) / 10 },
        trend,
        byPriority,
        byStatus,
        topContributors,
  ...
  app.get('/api/analytics/shopping', async (req, res) => {
    try {
      ...
      for (const item of purchasedItems) {
        if (item.purchasedById) {
          shopperMap.set(item.purchasedById, (shopperMap.get(item.purchasedById) || 0) + 1);
        }
      }
